...
def draft_handler(state: str, item_20_sample: str, fingerprint: dict) -> tuple[str, HandlerFn]:
    ...
    code = complete(SYSTEM_PROMPT, user_prompt)
    ...
    if "def parse(" not in code:
        raise ValueError("Agent 1 draft did not define a parse() function")

    sandbox_globals = {"__builtins__": {"len": len, "range": range, "str": str, "int": int}, "re": __import__("re")}
    sandbox_locals: dict = {}
